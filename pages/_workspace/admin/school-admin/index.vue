  ...
  <div class="schoolAdmin p-3">
    ...
    <template v-else>
      <b-button
        ...
        pill
        size="md"
        class="add-admin mb-4"
      >
        <b-icon icon="plus" />Register Admin
      </b-button>
                ...
                >
                  ...
                  <template #cell(actions)="data">
                    <div class="d-flex">
                      <b-button
                        :to="{
                          name: 'workspace-admin-school-admin-id',
                          params: {
        ...
        <template v-else>
          ...
          <div>
            ...
            <b-button
              ...
              @click="deleteInvokedSchoolAdmin"
            >
              Delete
            </b-button>
          </div>
...
export default {
  ...
  methods: {
    handleSchoolAdminEditModal(item) {
      this.adminId = item
    ...
    deleteInvokedSchoolAdmin() {
      this.isDeleting = true
      this.$apollo
        .mutate({
          mutation: DELETE_SCHOOL_ADMIN_MUTATION,
          variables: {
            id: parseInt(this.invokedForDelete.id),
            workspaceId: parseInt(this.mainWorkspace.id),
          },
          update: (store, { data: { deleteSchoolAdmin } }) => {
            try {
              ...
              const adminIndex = data.schoolAdmins.findIndex(
                (m) => m.id === this.invokedForDelete.id
              )

              if (adminIndex !== -1) {
                data.schoolAdmins.splice(adminIndex, 1)
              }
        ...
        .then(() => {
          Swal.fire({
            text: `School adnin updated successfully!`,
            position: 'top-right',
            color: '#fff',
            background: '#5cb85c',
            toast: false,
